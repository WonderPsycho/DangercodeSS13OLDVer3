/datum/game_mode/blob/proc/send_intercept(var/report = 1)
	...
	switch(report)
		...
		if(1)
			...
			intercepttext += "<FONT size = 3><B>Alliance of Racial Federation Update</B>: Biohazard Alert.</FONT><HR>"
			...
			intercepttext += "Alliance of Racial Federation has issued a directive 7-10 for [station_name()]. The station is to be considered quarantined.<BR>"
