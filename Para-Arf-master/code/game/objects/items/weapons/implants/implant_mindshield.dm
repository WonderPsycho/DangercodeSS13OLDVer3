...
/obj/item/weapon/implant/mindshield/get_data()
	var/dat = {"<b>Implant Specifications:</b><BR>
				<b>Name:</b> Alliance of Racial Federation Employee Management Implant<BR>
...
/obj/item/weapon/implant/mindshield/implant(mob/target)
	if(..())
		if(target.mind in ticker.mode.head_revolutionaries || is_shadow_or_thrall(target))
			target.visible_message("<span class='warning'>[target] seems to resist the implant!</span>", "<span class='warning'>You feel the corporate tendrils of The Alliance of Racial Federation try to invade your mind!</span>")
			removed(target, 1)
			qdel(src)
			return -1
		...
		if(target.mind in ticker.mode.cult)
			to_chat(target, "<span class='warning'>You feel the corporate tendrils of The Alliance of Racial Federation try to invade your mind!</span>")
		else
...
/obj/item/weapon/implant/mindshield/removed(mob/target, var/silent = 0)
	if(..())
		if(target.stat != DEAD && !silent)
			to_chat(target, "<span class='boldnotice'>You feel a sense of liberation as The Alliance of Racial Federation's grip on your mind fades away.</span>")
		return 1
	return 0
