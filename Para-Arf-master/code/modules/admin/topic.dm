/datum/admins/Topic(href, href_list)
	...
	if(href_list["makeAntag"])
		...
		switch(href_list["simplemake"])
			...
			if("queen_small")		M.change_mob_type( /mob/living/carbon/alien/humanoid/queen , null, null, delmob, 1 )
			...
			if("alien_admin")		M.change_mob_type( /mob/living/carbon/alien/humanoid/admin , null, null, delmob, 1 )
	...
	//Watchlist
		...
		switch(href_list["secretsfun"])
			...
			if("unlimitedpower")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","UP")
				log_admin("[key_name(usr)] turned all SMESs into magical SMESs", 1)
				message_admins("<span class='notice'>[key_name_admin(usr)] made all SMESs powered</span>", 1)
				unlimited_power()
			if("prisonwarp")
				if(!ticker)
