	...
	if(!oldcid)
		...
		if(oldcid != computer_id)
			...
			if(!cidcheck_failedckeys[ckey])
				...
				send2adminirc("[key_name(src)] has been detected as using a CID randomizer. Connection rejected.")
			...
			if(cidcheck_failedckeys[ckey])
				...
				send2adminirc("[key_name(src)] has been allowed to connect after showing they removed their cid randomizer.")
