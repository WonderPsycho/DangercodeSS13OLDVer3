...
/mob/living/proc/handle_micro_bump_other(var/mob/living/tmob)
	...
	if(src.a_intent == INTENT_DISARM && src.canmove && !src.buckled)
	...
	if(src.a_intent == INTENT_GRAB && src.canmove && !src.buckled)
		if((src.get_effective_size() - tmob.get_effective_size()) >= 0.75)
			now_pushing = 0
			src.forceMove(tmob.loc)
	...
	if(src.a_intent == INTENT_HARM && src.canmove && !src.buckled)
		if((src.get_effective_size() - tmob.get_effective_size()) >= 0.75)
			now_pushing = 0
			src.forceMove(tmob.loc)
			tmob.adjustStaminaLoss(10)
